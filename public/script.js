...
class QuizApp {
  constructor() {
    ...
    this.overallSetQuizScore = {
      // Overall score for the entire set quiz
      correct: 0,
      total: 0,
      percentage: 0,
    };
  ...
  addEventListeners() {
    ...
    if (this.quizModeToggle) {
      ...
      console.error("Event Listener Error: Element #quiz-mode-toggle not found, cannot add dropdown listener.");
    }
    ...
    if (this.quizSourceToggle) {
      ...
      console.error("Event Listener Error: Element #quiz-source-toggle not found, cannot add dropdown listener.");
    }
    ...
    if (this.quizTypeToggle) {
      ...
      console.error("Event Listener Error: Element #quiz-type-toggle not found, cannot add dropdown listener.");
    }
    ...
    if (this.prevQuestionBtn) {
      ...
      console.error("Event Listener Error: Element #prev-question-btn not found.");
    }

    if (this.nextQuestionBtn) {
      this.nextQuestionBtn.addEventListener("click", this.goToNextQuestion.bind(this));
    } else {
      console.error("Event Listener Error: Element #next-question-btn not found.");
    }
    ...
    if (this.setQuizTimeInput) {
      ...
      console.error("Event Listener Error: Element #set-quiz-time-input not found.");
    }
  }
  ...
  loadQuizNavigation(filterType = "all") {
    ...
    quizzesToLoad.forEach((quiz, index) => {
      ...
      } else if (this.quizScores[quiz.id]) {
        // For single mode or set mode after submission
        this.applyCompletionStatusToNavItem(listItem, quiz.id);
      } else if (this.setQuizScoreData[quiz.id]) {
        // For set mode after submission (individual scores)
        this.applyCompletionStatusToNavItem(listItem, quiz.id);
      }
    });
  ...
  applyCompletionStatusToNavItem(navItem, quizId) {
    ...
    if (scoreInfo) {
      ...
      if (!scoreSpan) {
        ...
        separator.classList.add("quiz-nav-score-separator");
        // Insert the line break after the title span, before the score span
        const titleSpan = navItem.querySelector(".quiz-nav-title");
        if (titleSpan) {
          titleSpan.after(separator); // Insert after the title span
          separator.after(scoreSpan); // Insert score span after the separator
        } else {
  ...
  resetQuizState() {
    ...
    if (this.quizMode === "set" && !this.quizSubmitted) {
      // Only disable if not in review mode
      if (this.prevQuestionBtn) this.prevQuestionBtn.disabled = true;
      if (this.nextQuestionBtn) this.nextQuestionBtn.disabled = true;
    }
  }
  ...
  loadQuiz(quizId) {
    ...
    if (this.quizNav) {
      Array.from(this.quizNav.children).forEach((li) => {
        ...
        if (li.dataset.quizId == quizId) {
          // Use == for comparison as dataset is string
    ...
    if (currentQuiz.questionType === "single-choice" || currentQuiz.questionType === "fill-in-blank") {
      const optionsToHighlight = currentQuiz.questionType === "single-choice" ? currentQuiz.options : currentQuiz.options[0] ? currentQuiz.options[0].blanks : []; // Use blanks for fill-in-blank
    ...
    if (this.quizContentElement) {
      this.quizContentElement.querySelectorAll(".question-num-in-text").forEach((span) => {
        span.addEventListener("click", (event) => {
          ...
          const targetBlock = this.optionsContainer.querySelector(`.fill-in-blank-input[data-blank-index="${qIndex}"]`) || this.optionsContainer.querySelector(`.question-block[data-question-index="${qIndex}"]`); // Fallback for general question blocks
          if (targetBlock) {
  ...
  updateMatchingUserPairsDisplay(quizId, questionIndex) {
    ...
    const currentQuiz = this.quizMode === "single" ? this.currentQuizData.questions[this.currentQuizIndex] : this.setQuizQuestions[this.currentSetQuizQuestionIndex];
    ...
    allLeftItems.forEach((leftValue) => {
      ...
      if (rightValue) {
        // Only display if a pair exists for this left item
  ...
  startQuiz() {
    ...
    if (this.currentVideoElement) {
      ...
      if (this.quizStarted) {
        // Only play if quiz is actively running (not review)
    ...
    if (currentQuiz.questionType === "fill-in-blank") {
      ...
      this.optionsContainer.querySelectorAll(".matching-item").forEach((item) => {
        ...
        if (item.classList.contains("left-item")) {
          ...
          if (!userMappings[item.dataset.value]) {
            // Only if not already paired
            item.draggable = true;
          } else {
            // If already paired, keep it disabled
            item.classList.add("disabled");
            item.style.pointerEvents = "none";
            item.draggable = false;
          }
  ...
  handleSubmitOrReattempt() {
    const currentQuizDataFromState = this.quizMode === "single" ? this.currentQuizData && this.currentQuizData.questions[this.currentQuizIndex] : this.setQuizQuestions && this.setQuizQuestions[this.currentSetQuizQuestionIndex]; // Get the current question for context
  ...
  evaluateSetQuizSubmission() {
    ...
    this.setQuizQuestions.forEach((quiz) => {
      ...
      if (userAnswersForThisQuiz) {
        // Check if there's an entry for this quizId
        for (const key in userAnswersForThisQuiz) {
          if (Array.isArray(userAnswersForThisQuiz[key])) {
            // For multi-choice/fill-in-blank (array of answers/blanks)
            if (userAnswersForThisQuiz[key].some((val) => (typeof val === "string" ? val.trim() !== "" : val !== undefined && val !== null))) {
              hasAnyAnswer = true;
              break;
            }
          } else if (typeof userAnswersForThisQuiz[key] === "object" && userAnswersForThisQuiz[key] !== null) {
            // For matching (object of pairs)
            if (Object.keys(userAnswersForThisQuiz[key]).length > 0) {
              hasAnyAnswer = true;
              break;
            }
          } else if (userAnswersForThisQuiz[key] !== undefined && userAnswersForThisQuiz[key] !== null && userAnswersForThisQuiz[key] !== "") {
            // For single-choice/true-false (single string)
            hasAnyAnswer = true;
            break;
          }
        }
      ...
      if (!hasAnyAnswer) {
        // If no answers provided for any part of this quiz
